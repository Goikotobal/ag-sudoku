...
export default function AISudoku() {
    ...
    // 🏆 PRO MODE - Unlocked for everyone! (No user profiles yet)
    const [isProUnlocked, setIsProUnlocked] = useState(true) // 🔓 UNLOCKED!
    const [bestExpertTime, setBestExpertTime] = useState<number | null>(null)
    
    // 📱 SCREEN HEIGHT DETECTION - For better spacing on tall screens
    const [screenHeightClass, setScreenHeightClass] = useState<'small' | 'medium' | 'large'>('medium')
    ...
    // 📱 Detect screen height for dynamic spacing
    useEffect(() => {
        const checkScreenHeight = () => {
            const height = window.innerHeight
            if (height < 700) {
                setScreenHeightClass('small')   // Compact phones
            } else if (height < 850) {
                setScreenHeightClass('medium')  // Normal phones (iPhone 14, etc)
            } else {
                setScreenHeightClass('large')   // Large phones (S21 Ultra, iPhone Pro Max)
            }
        }
    ...
    // 🏆 PRO UNLOCK: Load Pro unlock status from localStorage
    useEffect(() => {
        try {
            const proUnlocked = localStorage.getItem('ag-sudoku-pro-unlocked')
    ...
    useEffect(() => {
        ...
        const saveGame = () => {
            try {
                ...
                localStorage.setItem('ag-sudoku-save', JSON.stringify(gameState))
            } catch (error) {
    ...
    }, [board, solution, initialBoard, timer, mistakes, hintsRemaining, currentDifficulty, notes, showWinModal, showGameOverModal])
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (/^[1-9]$/.test(key)) {
                ...
                const btn = document.querySelector(`button[data-number="${num}"]`) as HTMLButtonElement
                console.log('⌨️ Looking for button:', num, 'Found:', btn, 'Disabled:', btn?.disabled)
                if (btn && !btn.disabled) {
            ...
            // Delete/Backspace/0
                ...
                e.preventDefault()
                const btn = document.querySelector(`button[data-number="0"]`) as HTMLButtonElement
    ...
    const findBestHintWithExplanation = useCallback((currentBoard: number[][]) => {
                ...
                if (currentBoard[row][col] === 0) {
                    ...
                    if (validNumbers.length === 1) {
                        return {
                            ...
                            conflictingCells: [...new Set(conflictingCells)] // Remove duplicates
                        }
                    }
                }
            }
    ...
    const autoFinishPuzzle = useCallback(async () => {
        ...
        if (isComplete) {
            ...
            if (currentDifficulty === 'expert' && timer < 900) { // 900 seconds = 15 minutes
                if (!isProUnlocked) {
                    setIsProUnlocked(true)
                    localStorage.setItem('ag-sudoku-pro-unlocked', 'true')
                    console.log('🎉 PRO MODE UNLOCKED! Expert completed in', Math.floor(timer / 60), 'minutes!')
                }
                
                // Update best Expert time
                if (!bestExpertTime || timer < bestExpertTime) {
                    setBestExpertTime(timer)
                    localStorage.setItem('ag-sudoku-best-expert-time', timer.toString())
                }
            }
        }
    ...
    const renderHeader = () => (
        <div
            style={{
                ...
                width: isDesktop ? "100%" : "96vw", // 🔥 MATCH BOARD!
                ...
                <div>
                    ...
                    <div
                        style={{
                            ...
                            animation:
                                "auroraGradient 4s ease-in-out infinite",
                            color: "white",
                            padding: "3px 8px",
                            borderRadius: 20,
                ...
                return (
                    <button
                        ...
                        style={{
                            ...
                            fontWeight: 600,
                            fontSize: isDesktop ? "12px" : "clamp(10px, 2.5vw, 11px)", // 🔥 SMALLER!
                            color:
                                currentDifficulty === diff
                                    ? "white"
                                    : isProLocked
    ...
    const renderSudokuGrid = () => (
        <div
            style={{
                ...
                pointerEvents:
                    isPaused || isAISolving ? "none" : "auto",
                opacity: isPaused ? 0.5 : 1,
                transition: "all 0.3s ease",
                maxWidth: isDesktop ? "500px" : "none",
                width: isDesktop ? "500px" : "96vw", // 🔥 MAXIMIZED: 96% of screen width!
                margin: "0 auto",
        ...
        >
            {board.map((row, rowIndex) =>
                row.map((cell, colIndex) => {
                    ...
                    const isInCompletedBox =
                        completedBoxes.has(boxIndex)
                    const isCompletedNum =
                        cell !== 0 && completedNumber === cell

                    // Check if recently placed (for color fade)
                    const cellKey = `${rowIndex}-${colIndex}`
                    const isRecentlyPlaced =
                        recentlyPlacedCells.has(cellKey)
                    ...
                    const isHintCell = showHintHighlights && 
                        showHintHighlights.cell.row === rowIndex && 
                        showHintHighlights.cell.col === colIndex
                    const isConflictCell = showHintHighlights && 
                    ...
                    return (
                        <div
                            ...
                            style={{
                                ...
                                alignItems: "center",
                                justifyContent: "center",
                                fontSize: isDesktop ? 28 : "clamp(18px, 5vw, 28px)", // 🔥 BIGGER NUMBERS!
                                fontWeight: 700,
                                cursor: "pointer",
                                color: isGiven
                                    ? "#2d3748"
                                ...
                                outline: isHintCell
                                    ...
                                    : isSelected
                                      ? notesMode
                                          ? "4px solid #a855f7" // 🔥 THICKER outline!
                                          : "4px solid #10b981" // 🔥 THICKER outline!
                                      : "none",
                                outlineOffset: "-4px", // 🔥 Adjusted for thicker outline
                                boxSizing: "border-box",
                                boxShadow: isSelected 
                                    ? notesMode
                                        ? "0 0 0 4px rgba(168, 85, 247, 0.2), inset 0 0 0 1px rgba(168, 85, 247, 0.3)" // 🔥 GLOW!
                                        : "0 0 0 4px rgba(16, 185, 129, 0.2), inset 0 0 0 1px rgba(16, 185, 129, 0.3)" // 🔥 GLOW!
                                    : "none",
                                transition:
    ...
    const getSpacing = () => {
        if (isDesktop) {
            return {
                boardToNumbers: 12,   // Space between board and number pad
                numbersToActions: 8,  // Space between number pad and action buttons
                actionsToBottom: 0,   // Space after action buttons
            }
        }
        
        // Mobile spacing based on screen height
        switch (screenHeightClass) {
            case 'small':  // < 700px (compact)
                return {
                    boardToNumbers: 8,
                    numbersToActions: 6,
                    actionsToBottom: 4,
                }
            case 'medium': // 700-850px (normal)
                return {
                    boardToNumbers: 24,  // 🔥 INCREASED even more! (was 16)
                    numbersToActions: 14, // 🔥 INCREASED even more! (was 10)
                    actionsToBottom: 16,  // 🔥 INCREASED even more! (was 12)
                }
            case 'large':  // > 850px (tall screens like S21 Ultra)
                return {
                    boardToNumbers: 48,   // 🔥 MAXIMUM SPACING! (was 32)
                    numbersToActions: 24,  // 🔥 MAXIMUM SPACING! (was 16)
                    actionsToBottom: 32,   // 🔥 MAXIMUM SPACING! (was 24)
                }
        }
    }
        ...
        <>
            ...
            <div
                style={{
                    ...
                    marginBottom: spacing.numbersToActions, // 🔥 DYNAMIC SPACING!
            ...
            >
                {[1, 2, 3, 4, 5, 6, 7, 8, 9, 0].map((num) => {
                    ...
                    const isSelected = selectedNumber === num // 🔥 NEW: Track if this number is selected!
                    ...
                    return (
                        <button
                            ...
                            style={{
                                ...
                                border: isSelected 
                                    ? "3px solid #a855f7" // 🔥 PURPLE BORDER when selected!
                                    : "2px solid #e2e8f0",
                                background: isComplete
                                    ? "#f7fafc"
                                    : isSelected 
                                    ? "linear-gradient(135deg, #a855f7 0%, #ec4899 100%)" // 🔥 GRADIENT when selected!
                                    : "white",
                                borderRadius: 8, // 🔥 Slightly more rounded
                                cursor: isComplete
                                    ? "not-allowed"
                                    : "pointer",
                                fontSize: isDesktop ? "28px" : "clamp(18px, 4.1vw, 22px)", // 🔥 75% SMALLER! (was 24-30px)
                                fontWeight: 700,
                                color: isComplete 
                                    ? "#cbd5e0" 
                                    : isSelected 
                                ...
                                alignItems: "center",
                                gap: 1,
                                minHeight: isDesktop ? "60px" : screenHeightClass === 'large' ? "39px" : "35px", // 🔥 75% SMALLER! (was 52/46px)
                                touchAction: "manipulation",
                                WebkitTapHighlightColor: "transparent",
                                transform: isSelected ? "scale(0.95)" : "scale(1)", // 🔥 Slight press effect!
                                boxShadow: isSelected 
                                    ? "0 4px 12px rgba(168, 85, 247, 0.4)" // 🔥 Glow when selected!
                                    : "none",
                            }}
                        >
                            <span>{num === 0 ? "✕" : num}</span>
                            {num !== 0 && (
                                <span
                                    style={{
                                        fontSize: isDesktop ? "13px" : "clamp(11px, 2.5vw, 14px)", // 🔥 BIGGER!
                                        fontWeight: 600, // 🔥 BOLDER!
                                        color: isSelected 
                                            ? "rgba(255, 255, 255, 0.9)" // 🔥 WHITE remaining count when selected!
                                            : remaining === 0
                                            ? "#cbd5e0"
                                            : "#94a3b8",
                                    }}
            ...
            <div style={{ marginBottom: spacing.numbersToActions }} /> {/* 🔥 DYNAMIC: Between number pad and action buttons */}
            ...
            >
                ...
                <button
                    ...
                    style={{
                        ...
                        cursor: isAISolving ? "not-allowed" : "pointer",
                        fontWeight: 600,
                        fontSize: isDesktop ? "12px" : "clamp(9px, 2.2vw, 11px)", // 🔥 SMALLER!
                ...
                >
                    <span style={{ fontSize: isDesktop ? "16px" : "14px" }}>✏️</span> {/* 🔥 SMALLER ICON! */}
                    <span style={{ fontSize: isDesktop ? "9px" : "clamp(7px, 1.8vw, 8px)" }}>
                        Notes
                    </span>
                </button>
                ...
                <button
                    ...
                    style={{
                        ...
                        cursor: isAISolving ? "not-allowed" : "pointer",
                        fontWeight: 600,
                        fontSize: isDesktop ? "12px" : "clamp(9px, 2.2vw, 11px)", // 🔥 BIGGER!
                        transition: "all 0.2s",
                        display: "flex",
                        flexDirection: "column",
                        alignItems: "center",
                ...
                >
                    <span style={{ fontSize: isDesktop ? "16px" : "14px" }}>{isPaused ? "▶️" : "⏸️"}</span>
                    <span style={{ fontSize: isDesktop ? "9px" : "clamp(7px, 1.8vw, 8px)" }}> {/* 🔥 BIGGER! */}
                ...
                >
                    <span style={{ fontSize: isDesktop ? "16px" : "14px" }}>💡</span>
                    <span style={{ fontSize: isDesktop ? "9px" : "clamp(7px, 1.8vw, 8px)" }}> {/* 🔥 BIGGER! */}
                        {rules.hints === 0
                            ? "None"
                            : hintsRemaining === 0
                ...
                >
                    <span style={{ fontSize: isDesktop ? "16px" : "14px" }}>🎮</span>
                    <span style={{ fontSize: isDesktop ? "9px" : "clamp(7px, 1.8vw, 8px)" }}>New</span> {/* 🔥 BIGGER! */}
                </button>
            </div>
            
            {/* Bottom spacing on large screens */}
            {!isDesktop && <div style={{ marginBottom: spacing.actionsToBottom }} />}
        </>
    )
    ...
    return (
        ...
        >
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
                ...
                {/* Spacing between difficulty selector and board */}
                {!isDesktop && <div style={{ marginBottom: spacing.boardToNumbers * 0.6 }} />} {/* 🔥 60% of bottom spacing */}
                ...
                {isDesktop ? (
                    ...
                    <div style={{
                        ...
                        {/* 🔥 CRITICAL: Spacing between board and number pad! */}
                        <div style={{ marginBottom: spacing.boardToNumbers }} />
                ...
                >
                    ...
                    >
                        ...
                        <div style={{
                            ...
                            <div style={{ textAlign: "left" }}>
                                ...
                                <div style={{
                                    ...
                                    Row {hintExplanation.cell.row + 1}, Col {hintExplanation.cell.col + 1}
                                </div>
                            </div>
                            <div style={{
                                width: 44,
                        ...
                        <div style={{
                            ...
                            <div style={{
                                ...
                                {[...Array(9)].map((_, i) => {
                                    ...
                                    const hasConflict = hintExplanation.conflictingCells && 
                                        hintExplanation.conflictingCells.length > 0 && 
                                        (row === 1 || col === 1) && !isTarget;
                                    ...
                                    return (
                                        ...
                                        >
                                            {isTarget ? hintExplanation.value : hasConflict ? "✕" : ""}
                                        </div>
                                    );
                                })}
                            </div>
                            ...
                            <div style={{ 
                                ...
                                {hintExplanation.conflictingCells && hintExplanation.conflictingCells.length > 0 && (
                                    <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "center" }}>
                                        ...
                                        <span>Conflicts block other options</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div style={{
                            ...
                            <div style={{ fontSize: 10, fontWeight: 600, color: "#10b981", marginBottom: 6 }}>
                                ✨ After clicking "Apply Hint":
                            </div>
                            <div style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 10, color: "#64748b" }}>
                                <div>• Yellow cell shows where <strong>{hintExplanation.value}</strong> was placed</div>
                                {hintExplanation.conflictingCells && hintExplanation.conflictingCells.length > 0 && (
                                    <div>• Red cells show blocking numbers</div>
                                )}
                                <div style={{ color: "#10b981", fontWeight: 600, fontStyle: "italic", marginTop: 2 }}>
            ...
            {isPaused && (
                ...
                >
                    <div
                        ...
                        onClick={(e) => e.stopPropagation()}
                    >
                        <div style={{ fontSize: isDesktop ? 48 : 36, marginBottom: isDesktop ? 20 : 10 }}>⏸️</div>
                        <h2
                            style={{
                                color: "#2d3748",
                        ...
                        >
                            <div
                                style={{
                                    background: "linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%)", // 🎨 Light green!
                        ...
                        >
                            <button
                                ...
                                style={{
                                    padding: isDesktop ? "14px 24px" : "12px 20px", // 🔥 MOBILE: Compact
                                    border: "none",
                                    borderRadius: 10,
                                    cursor: "pointer",
                                    fontWeight: 700,
                            ...
                            <button
                                ...
                                style={{
                                    padding: isDesktop ? "12px 24px" : "10px 20px", // 🔥 MOBILE: Compact
                                    border: "2px solid #e2e8f0",
                                    borderRadius: 10,
                                    cursor: "pointer",
                                    fontWeight: 600,
                            ...
                            <button
                                onClick={() => {
                                    if (
                                        ...
                                        setIsPaused(false)
                                        // Clear the game and go back to initial state
